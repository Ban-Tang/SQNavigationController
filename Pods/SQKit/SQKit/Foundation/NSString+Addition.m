...
- (NSAttributedString *)stringByAppendImageNamed:(NSString *)imageName {
    return [self stringByInsertImage:[UIImage imageNamed:imageName] atIndex:self.length];
}
...
- (NSAttributedString *)stringByInsertImage:(UIImage *)image atIndex:(NSInteger)index {
    ...
    if (image) {
        ...
        attach.image = image;
        attach.bounds = CGRectMake(0, 0, ceil(image.size.width), ceil(image.size.height));
