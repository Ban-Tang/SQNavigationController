...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} bt_net_interface_counter;


static uint64_t bt_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t bt_net_counter_get_by_type(bt_net_interface_counter *counter, BTNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & BTNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & BTNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & BTNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & BTNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & BTNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & BTNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static bt_net_interface_counter bt_get_net_interface_counter() {
    ...
    bt_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = bt_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = bt_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)getNetworkTrafficBytes:(BTNetworkTrafficType)types {
    bt_net_interface_counter counter = bt_get_net_interface_counter();
    return bt_net_counter_get_by_type(&counter, types);
}
